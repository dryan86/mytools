...
def frames_from_data(filename, format):
    ...
    if format == 'plist':
        ...
        for k, v in frames:
            ...
            offsetKey = 'spriteOffset'
            sourceSizeKey = 'spriteSourceSize'
            if frame.has_key('frame'):
                rectKey = 'frame'
                rotateKey = 'rotated'
                offsetKey = 'offset'
                sourceSizeKey = 'sourceSize'
            frame['rotated'] = frame[rotateKey]

            rectlist = to_list(frame[rectKey])
            width = int(rectlist[3] if frame[rotateKey] else rectlist[2])
            height = int(rectlist[2] if frame[rotateKey] else rectlist[3])
            frame['box'] = (
                int(rectlist[0]),
                int(rectlist[1]),
            ...
            real_rectlist = to_list(frame[sourceSizeKey])
            real_width = int(real_rectlist[1] if frame[rotateKey] else real_rectlist[0])
            real_height = int(real_rectlist[0] if frame[rotateKey] else real_rectlist[1])
            real_sizelist = [real_width, real_height]
            frame['real_sizelist'] = real_sizelist
            offsetlist = to_list(frame[offsetKey])
            offset_x = int(offsetlist[1] if frame[rotateKey] else offsetlist[0])
            offset_y = int(offsetlist[0] if frame[rotateKey] else offsetlist[1])
            frame['result_box'] = (
...
# move done file
def moveDonefile(fileName):
    command = "mv '%s' '%s'" % (fileName, "./done/")
    os.system(command)
